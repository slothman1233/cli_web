...
class test {
     ...
     */
    async updateTest({ name: newName, weight: newWeight }: { name: string, weight: number }, { name }: { name: string }) {
        // 拼接修改内容
        let updateData: any = {}
     ...
     * @param {number} id id
     */
    async updateTestById({ name: newName, weight: newWeight }: { name: string, weight: number }, id: number) {
        let updateData: any = {}
        if (newName) { updateData.name = newName }
        if (newWeight) { updateData.weight = newWeight }
