 ...
 */
async function addTest({ userId, age, garden = 3 }: { userId: number, age: number, garden: number }) {
 ...
 */
async function updateTest({ age, garden }: { age: number, garden: number }, { userId }: { userId: number }) {
    // 拼接修改内容
    let updateData: any = {}
 ...
 * @param {number} id id
 */
async function updateTestById({ age, garden }: { age: number, garden: number }, id: number) {
    let updateData: any = {}
    if (age) { updateData.age = age }
    if (garden) { updateData.garden = garden }
...
export { addTest, updateTest, updateTestById, getTestList, getTestById, delTestById, BelongsToTest }